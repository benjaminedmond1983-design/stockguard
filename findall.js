...
while (i < len) {
  ...
  if (ch === '<') {
    ...
    if (proceed) {
      ...
      if (closing) {
        if (name === 'Fragment') {
          // check if it matches top of stack; if top is Fragment, pop normally
          if (stack.length && stack[stack.length-1].name === 'Fragment') {
            stack.pop();
          } else {
            // stray closing fragment - log and skip (don't pop)
            strayClosers.push(startLine);
          }
          continue;
        }
        if (stack.length === 0) {
          console.log(`FATAL: Extra closing </${name}> at line ${startLine}, stack empty`);
          process.exit();
        }
        const top = stack[stack.length - 1];
        if (top.name !== name) {
          console.log(`FATAL MISMATCH at line ${startLine}: found </${name}> but top of stack is <${top.name}> opened at line ${top.line}`);
          console.log('Last 15 on stack:', stack.slice(-15).map(s => `${s.name}@${s.line}`).join(', '));
          process.exit();
        }
        stack.pop();
      } else if (!selfClosing) {
        if (name === 'Fragment') strayOpeners.push(startLine);
...
console.log('\nStray Fragment openers (<>) at lines:', strayOpeners);
console.log('Stray Fragment closers (</>) at lines:', strayClosers);
