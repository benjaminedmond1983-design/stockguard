...
function AppInner({role,onLogout,TABS,userId}){
  ...
  function handleSaleScan(raw){const code=raw.trim();if(!code)return;if(window._sgStopCamera)window._sgStopCamera();setCameraActive(false);setCameraError("");setScanMode(false);const existing=inventory.find(i=>i.sku===code||i.sku.toLowerCase()===code.toLowerCase());if(existing){setSaleForm(f=>({...f,sku:existing.sku}));}else{setSaleForm(f=>({...f,sku:code}));}}function handleRecScan(raw){const code=raw.trim();setRecScanMode(false);setRecCameraActive(false);if(window._sgStopRecCamera)window._sgStopRecCamera();if(!code)return;const existing=inventory.find(i=>i.sku===code||i.sku.toLowerCase()===code.toLowerCase());if(existing){setRecForm(r=>({...r,sku:existing.sku,name:existing.name,category:existing.category,supplier:existing.supplier,unitCost:existing.unitCost,sellingPrice:existing.sellingPrice,location:existing.location}));}else{setRecForm(r=>({...r,sku:code}));}}
          ...
          <div style={{background:"#0D1F2D",borderRadius:12,padding:"14px 16px",marginBottom:14,overflow:"hidden"}}>
            {cameraActive&&(
              <div style={{position:"relative",marginBottom:12}}>
                ...
                <div style={{position:"absolute",bottom:8,left:0,right:0,textAlign:"center",color:"rgba(255,255,255,0.8)",fontSize:12}}><button onClick={async()=>{const vid=document.getElementById("sg-sale-camera-feed");if(!vid||!vid.videoWidth){setCameraError("Camera not ready.");return;}setCameraError("Scanning...");try{if("BarcodeDetector" in window){const det=new BarcodeDetector({formats:["ean_13","ean_8","code_128","code_39","qr_code","upc_a","upc_e"]});const codes=await det.detect(vid);if(codes.length>0){handleSaleScan(codes[0].rawValue);return;}}setCameraError("No barcode found. Tap again.");}catch(e){setCameraError("No barcode found. Tap again.");}}} style={{background:"#0D7E6E",border:"none",color:"#fff",padding:"10px 28px",borderRadius:20,fontSize:15,fontWeight:700,cursor:"pointer"}}>📷 Tap to scan</button></div>
              </div>
            )}
