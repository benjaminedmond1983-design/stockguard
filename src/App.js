...
function AppInner({role,onLogout,TABS,userId}){
  ...
  async function startRecCameraScan(){setRecCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}});setRecCameraActive(true);await new Promise(res=>setTimeout(res,150));const video=document.getElementById("sg-rec-camera-feed");if(!video){stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);return;}video.srcObject=stream;await video.play();const recReader=new BrowserMultiFormatReader();window._sgStopRecCamera=()=>{try{recReader.reset();}catch(e){}stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);};recReader.decodeFromVideoElement(video,(result)=>{if(result){window._sgStopRecCamera();handleRecScan(result.getText());}});}catch(e){setRecCameraError(e.name==="NotAllowedError"?"Camera access denied.":"Camera not available. Type SKU below.");setRecCameraActive(false);}}
    try{
