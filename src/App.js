...
function shortDate(ds){const[,,day]=ds.split("-");const d=new Date(ds+"T00:00:00");return d.toLocaleDateString("en-US",{weekday:"short"})+" "+parseInt(day);}
function buildInitAudit(){return[
...
function statusBadge(qty,min){if(qty===0)return{label:"Out of stock",bg:"#FCEBEB",color:"#A32D2D"};if(qty<min*0.3)return{label:"Critical",bg:"#FCEBEB",color:"#A32D2D"};if(qty<min)return{label:"Low",bg:"#FAEEDA",color:"#854F0B"};return{label:"OK",bg:"#EAF3DE",color:"#3B6D11"};}
function marginBadge(cost,sell){if(!sell||sell<=cost)return{label:"No margin",pct:0,profit:0,bg:"#F5F5F5",color:"#888"};const profit=sell-cost,pct=(profit/sell)*100;if(pct>=40)return{label:`${pct.toFixed(0)}%`,pct,profit,bg:"#EAF3DE",color:"#3B6D11"};if(pct>=20)return{label:`${pct.toFixed(0)}%`,pct,profit,bg:"#FAEEDA",color:"#854F0B"};return{label:`${pct.toFixed(0)}%`,pct,profit,bg:"#FCEBEB",color:"#A32D2D"};}
    ...
    <div style={{minHeight:"100vh",background:"#EEF2F7",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"40px 36px",width:360,boxShadow:"0 4px 24px rgba(0,0,0,0.10)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,marginBottom:28}}>
          {SG_LOGO}
          <div><div style={{fontWeight:700,fontSize:22,color:"#1B2B4B",lineHeight:1}}>StockGuard</div><div style={{fontSize:12,color:"#888",marginTop:2}}>Supply Chain Tracker</div></div>
        </div>
        ...
        {mode!=="forgot"&&<input placeholder="Password" type="password" value={password} onChange={e=>setPassword(e.target.value)} style={inp} onKeyDown={e=>e.key==="Enter"&&handleSubmit()}/>}
        ...
        <div style={{display:"flex",justifyContent:"center",gap:16,fontSize:12,color:"#666"}}>
          {mode==="signin"&&<><button onClick={()=>{setMode("signup");setError("");setMessage("");}} style={{background:"none",border:"none",color:"#185FA5",cursor:"pointer",fontSize:12}}>Create account</button><span>·</span><button onClick={()=>{setMode("forgot");setError("");setMessage("");}} style={{background:"none",border:"none",color:"#185FA5",cursor:"pointer",fontSize:12}}>Forgot password</button></>}
          {mode!=="signin"&&<button onClick={()=>{setMode("signin");setError("");setMessage("");}} style={{background:"none",border:"none",color:"#185FA5",cursor:"pointer",fontSize:12}}>← Back to sign in</button>}
        </div>
        <div style={{marginTop:24,fontSize:11,color:"#aaa",fontStyle:"italic",textAlign:"center"}}>"Commit to the Lord whatever you do"<br/>Proverbs 16:3</div>
      </div>
    </div>
  );
}

function RoleScreen({onRole,onSignOut}){
  const [showPin,setShowPin]=useState(false);
  const [pin,setPin]=useState("");
  const [pinError,setPinError]=useState("");
  function tryOwnerPin(){if(pin===OWNER_PIN){onRole("owner");}else{setPinError("Incorrect PIN. Try again.");setPin("");}}
  ...
  if(showPin) return(
    <div style={{minHeight:"100vh",background:"#EEF2F7",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"40px 36px",width:340,boxShadow:"0 4px 24px rgba(0,0,0,0.10)"}}>
        ...
        <div style={{textAlign:"center",marginBottom:28}}><div style={{fontSize:32,marginBottom:8}}>🔐</div><div style={{fontWeight:700,fontSize:18,color:"#1B2B4B"}}>Owner PIN</div><div style={{fontSize:13,color:"#666",marginTop:4}}>Enter your PIN to access the owner dashboard</div></div>
        <input type="password" inputMode="numeric" maxLength={6} placeholder="Enter PIN" value={pin} onChange={e=>{setPin(e.target.value);setPinError("");}} onKeyDown={e=>e.key==="Enter"&&tryOwnerPin()} style={{width:"100%",padding:"12px 14px",borderRadius:8,border:`1px solid ${pinError?"#E05A5A":"#e0e0e0"}`,fontSize:18,textAlign:"center",letterSpacing:8,boxSizing:"border-box",marginBottom:8,outline:"none"}} autoFocus/>
        {pinError&&<div style={{color:"#A32D2D",fontSize:12,marginBottom:8,textAlign:"center"}}>{pinError}</div>}
        <button onClick={tryOwnerPin} style={{width:"100%",padding:"11px",borderRadius:8,border:"none",background:"#1B2B4B",color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer",marginTop:4}}>Unlock Owner View</button>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#EEF2F7",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <div style={{background:"#fff",borderRadius:16,padding:"48px 40px",width:380,boxShadow:"0 4px 24px rgba(0,0,0,0.10)",textAlign:"center"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,marginBottom:32}}>
          {SG_LOGO}
          <div style={{textAlign:"left"}}><div style={{fontWeight:700,fontSize:22,color:"#1B2B4B",lineHeight:1}}>StockGuard</div><div style={{fontSize:12,color:"#888",marginTop:2}}>Supply Chain Tracker</div></div>
        </div>
        ...
        <button onClick={()=>setShowPin(true)} style={{width:"100%",padding:"18px 20px",borderRadius:12,border:"2px solid #1B2B4B",background:"#1B2B4B",color:"#fff",cursor:"pointer",marginBottom:14,display:"flex",alignItems:"center",gap:16,textAlign:"left"}}>
          <span style={{fontSize:28}}>👔</span><div><div style={{fontWeight:700,fontSize:15}}>Owner</div><div style={{fontSize:12,opacity:0.7,marginTop:2}}>Full access — all reports, settings & tools</div></div><span style={{marginLeft:"auto",fontSize:18,opacity:0.6}}>→</span>
        </button>
        <button onClick={()=>onRole("cashier")} style={{width:"100%",padding:"18px 20px",borderRadius:12,border:"2px solid #0F6E56",background:"#0F6E56",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:16,textAlign:"left"}}>
          <span style={{fontSize:28}}>🧾</span><div><div style={{fontWeight:700,fontSize:15}}>Cashier</div><div style={{fontSize:12,opacity:0.7,marginTop:2}}>Sales, Receiving, Movements & Reorder alerts</div></div><span style={{marginLeft:"auto",fontSize:18,opacity:0.6}}>→</span>
        </button>
        <button onClick={onSignOut} style={{marginTop:20,background:"none",border:"none",color:"#aaa",fontSize:12,cursor:"pointer"}}>Sign out</button>
        <div style={{marginTop:16,fontSize:11,color:"#aaa",fontStyle:"italic"}}>"Commit to the Lord whatever you do"<br/>Proverbs 16:3</div>
      </div>
    </div>
  );
}
      ...
      {tab==="Dashboard"&&(<div>
        <div style={{display:"grid",gridTemplateColumns:"180px 1fr",gap:16,marginBottom:20}}>
          ...
          <div style={{background:"linear-gradient(135deg,#185FA5 0%,#0d3d6b 100%)",borderRadius:12,padding:"16px 20px",color:"#fff"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
              <div><div style={{fontSize:12,opacity:0.8,marginBottom:2}}>Today's Sales Summary</div><div style={{fontSize:28,fontWeight:700,lineHeight:1}}>${todaySales.revenue.toFixed(2)}</div><div style={{fontSize:12,opacity:0.8,marginTop:2}}>{revenueUp?"▲":"▼"} vs yesterday ${yesterdaySales.revenue.toFixed(2)}</div></div>
              <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>{[{label:"Profit today",val:`$${todaySales.profit.toFixed(2)}`,color:"#A8D57B"},{label:"Units sold",val:todaySales.units,color:"#7EC8E3"},{label:"Transactions",val:todaySales.transactions,color:"#F9C74F"}].map(s=>(<div key={s.label} style={{textAlign:"center"}}><div style={{fontSize:20,fontWeight:700,color:s.color}}>{s.val}</div><div style={{fontSize:11,opacity:0.8}}>{s.label}</div></div>))}</div>
            </div>
        ...
        <div style={{background:C.bg2,borderRadius:10,padding:"14px 16px",marginBottom:20}}>
          ...
          <div style={{display:"flex",alignItems:"flex-end",gap:6,height:110}}>
            {Object.entries(salesByDay).map(([day,data])=>{const isToday=day===today;const barH=maxRevenue>0?Math.max((data.revenue/maxRevenue)*72,data.revenue>0?6:2):2;return(<div key={day} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:4}}><div style={{fontSize:10,color:C.muted,fontWeight:isToday?700:400,marginBottom:4}}>${data.revenue>0?data.revenue.toFixed(0):0}</div><div style={{width:"100%",height:barH,background:isToday?"#185FA5":"#B8D4F0",borderRadius:"3px 3px 0 0"}}/><div style={{fontSize:10,color:isToday?C.text:C.muted,fontWeight:isToday?700:400,whiteSpace:"nowrap"}}>{shortDate(day)}</div></div>);})}
          </div>
        </div>
...
<div><label style={{fontSize:12,color:C.muted}}>Unit cost ($)</label><input type="number" placeholder="0.00" value={recForm.unitCost} onChange={e=>setRecForm(r=>({...r,unitCost:e.target.value}))} style={inp}/></div>
<div><label style={{fontSize:12,color:C.muted}}>Selling price ($)</label><input type="number" placeholder="0.00" value={recForm.sellingPrice} onChange={e=>setRecForm(r=>({...r,sellingPrice:e.target.value}))} style={inp}/></div>
