...
function AppInner({role,onLogout,TABS,userId,userEmail}){
  ...
  const [settings,setSettings]=useState({po_show_verse:true,business_name:'',business_address:'',po_logo_url:null,po_counter:1});
  ...
  useEffect(()=>{
    async function loadAll(){
      ...
      if(settingsData){setSettings({po_show_verse:settingsData.po_show_verse,business_name:settingsData.business_name||'',business_address:settingsData.business_address||'',po_logo_url:settingsData.po_logo_url||null,po_counter:settingsData.po_counter||1});setPOCounter(settingsData.po_counter||1);}
      else{const{data:ins}=await supabase.from("user_settings").insert({user_id:userId}).select().maybeSingle();if(ins){setSettings({po_show_verse:ins.po_show_verse,business_name:ins.business_name||'',business_address:ins.business_address||'',po_logo_url:ins.po_logo_url||null,po_counter:ins.po_counter||1});setPOCounter(ins.po_counter||1);}}
      setDbLoading(false);
    }
    loadAll();
  },[]);
  ...
  async function saveSettings(updates){
    ...
    setSettings(merged);
    await supabase.from("user_settings").update({po_show_verse:merged.po_show_verse,business_name:merged.business_name,business_address:merged.business_address,po_logo_url:merged.po_logo_url,po_counter:merged.po_counter,updated_at:new Date().toISOString()}).eq("user_id",userId);
  }
...
async function startCameraScan(){setCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}});setCameraActive(true);await new Promise(res=>setTimeout(res,300));const video=document.getElementById("sg-camera-feed");if(!video){stream.getTracks().forEach(t=>t.stop());setCameraActive(false);return;}video.srcObject=stream;await video.play();window._sgStopCamera=()=>{stream.getTracks().forEach(t=>t.stop());setCameraActive(false);};window._sgCaptureScan=async()=>{const vid=document.getElementById("sg-camera-feed");if(!vid||!vid.videoWidth){setScanFeedback({ok:false,msg:"Camera not ready yet."});return;}setScanFeedback({ok:false,msg:"Scanning..."});try{if("BarcodeDetector" in window){const det=new BarcodeDetector({formats:["ean_13","ean_8","code_128","code_39","qr_code","upc_a","upc_e"]});const codes=await det.detect(vid);if(codes.length>0){const scanned=codes[0].rawValue;if(window._sgStopCamera)window._sgStopCamera();window._sgSetSaleSku(scanned);window._sgStopCamera&&window._sgStopCamera();window._sgScanDone=true;return;}}setScanFeedback({ok:false,msg:"No barcode found. Tap again."});}catch(e){setScanFeedback({ok:false,msg:"No barcode found. ("+e.name+")"});}};}catch(e){alert("CAM ERR: "+e.name+": "+e.message);setCameraError(e.name==="NotAllowedError"?"Camera access denied. Please allow camera access.":"Camera not available. Use the text input below.");setCameraActive(false);}}
  ...
  async function handleReorder(item,qtyOverride){
    const suggestedQty=parseInt(qtyOverride)>0?parseInt(qtyOverride):Math.max(item.minQty*2-item.qty,10);
    const poNumber=`PO-${String(poCounter).padStart(4,"0")}`;const nextPOCounter=poCounter+1;setPOCounter(nextPOCounter);saveSettings({po_counter:nextPOCounter});
    const poRow={po_number:poNumber,status:"Draft",sku:item.sku,item_name:item.name,description:`Reorder for ${item.name} — SKU ${item.sku}`,supplier:item.supplier,qty:suggestedQty,unit_cost:item.unitCost,date:todayStr(),delivery_date:"",notes:"",created_from:"Reorder Center",user_id:userId};
    const{data}=await supabase.from("purchase_orders").insert(poRow).select().single();
  ...
  function handleCSVUpload(e){const file=e.target.files[0];if(!file)return;if(/\.xlsx?$/i.test(file.name)){setImportErrors(["This looks like an Excel file (.xlsx/.xls). Please save it as CSV (File → Save As → CSV) and upload the CSV instead."]);e.target.value="";return;}const reader=new FileReader();reader.onload=evt=>{try{const text=evt.target.result.replace(/^\ufeff/,"");const lines=text.trim().split(/\r?\n/);const headerLine=lines[0];const delim=headerLine.includes("\t")?"\t":((headerLine.match(/;/g)||[]).length>(headerLine.match(/,/g)||[]).length?";":",");const headers=headerLine.split(delim).map(c=>c.replace(/"/g,"").trim());const rows=lines.slice(1).filter(l=>l.trim()).map(line=>{const cols=line.split(delim).map(c=>c.replace(/"/g,"").trim());const obj={};headers.forEach((h,i)=>{obj[h]=cols[i]||"";});return obj;});const{parsed,errors}=parseRows(rows);setImportErrors(errors);setImportPreview(parsed);setImportStatus("preview");}catch(err){setImportErrors(["Failed to parse file: "+err.message]);}};reader.readAsText(file);e.target.value="";}
    ...
    <div style={{display:"flex",minHeight:"100vh",fontFamily:"system-ui,-apple-system,sans-serif",color:C.text,background:"#EEF2F7"}}>
      ...
      <div className="sg-sidebar" style={{width:SIDEBAR_W,minWidth:SIDEBAR_W,background:SIDEBAR,display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,height:"100vh",zIndex:100,overflowY:"auto"}}>
        ...
        <div style={{padding:"16px",borderTop:"1px solid rgba(255,255,255,0.08)"}}>
          ...
          <div style={{color:"rgba(255,255,255,0.35)",fontSize:10,textAlign:"center",lineHeight:1.5,fontStyle:"italic"}}>"Commit to the Lord whatever you do"<br/>Proverbs 16:3
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <div>
              ...
              <p style={{fontSize:12,margin:0,opacity:0.75}}>
                ...
                {tab==="Tax Center"&&"Year-end financial reports & accountant-ready exports"}
...
<div><label style={{fontSize:12,color:C.muted}}>PO number</label><input type="text" placeholder="PO-2201" value={recForm.po} onChange={e=>setRecForm(r=>({...r,po:e.target.value}))} style={inp}/></div>
      ...
      {tab==="Ask StockGuard"&&(<AskStockGuard inventory={inventory} />)}
        {tab==="Tax Center"&&(<TaxCenterTab inventory={inventory} audit={audit} pos={pos} />)}
      ...
      {tab==="clover"&&(<CloverTab inventory={inventory} onUpdateInventory={setInventory} userId={userId} supabase={supabase} />)}
    ...
    try {
      ...
      for (const order of data.orders) {
        for (const item of order.line_items) {
          ...
          if (audErr) throw new Error('Audit log failed for ' + invItem.sku + ': ' + audErr.message);
          deducted++;
        }
      }
