...
function AppInner({role,onLogout,TABS,userId}){
  ...
  async function startRecCameraScan(){setRecCameraError("");const reader=new BrowserMultiFormatReader();window._sgStopRecCamera=()=>{try{reader.reset();}catch(e){}setRecCameraActive(false);};try{setRecCameraActive(true);await new Promise(res=>setTimeout(res,200));const video=document.getElementById("sg-rec-camera-feed");if(!video){setRecCameraActive(false);return;}reader.decodeFromVideoDevice(undefined,video,(result)=>{if(result){window._sgStopRecCamera();handleRecScan(result.getText());}}).catch(err=>{setRecCameraError(err.name==="NotAllowedError"?"Camera access denied.":"Camera not available. Type SKU below.");setRecCameraActive(false);});}catch(e){setRecCameraError(e.name==="NotAllowedError"?"Camera access denied.":"Camera not available. Type SKU below.");setRecCameraActive(false);}}
    try{
