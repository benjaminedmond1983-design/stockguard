...
function AppInner({role,onLogout,TABS,userId}){
  ...
  function handleRecScan(raw){alert("handleRecScan called: "+raw);const code=raw.trim();setRecScanMode(false);setRecCameraActive(false);if(window._sgStopRecCamera)window._sgStopRecCamera();if(!code)return;const existing=inventory.find(i=>i.sku===code||i.sku.toLowerCase()===code.toLowerCase());if(existing){setRecForm(r=>({...r,sku:existing.sku,name:existing.name,category:existing.category,supplier:existing.supplier,unitCost:existing.unitCost,sellingPrice:existing.sellingPrice,location:existing.location}));}else{setRecForm(r=>({...r,sku:code}));}else{setRecForm(r=>({...r,sku:code}));}}
