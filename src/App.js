...
function AppInner({role,onLogout,TABS,userId}){
  ...
  async function startRecCameraScan(){setRecCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}});setRecCameraActive(true);await new Promise(res=>setTimeout(res,200));const video=document.getElementById("sg-rec-camera-feed");if(!video){stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);return;}video.srcObject=stream;await video.play();await new Promise(res=>setTimeout(res,1500));const canvas=document.createElement("canvas");const ctx=canvas.getContext("2d");const useDetector="BarcodeDetector" in window;const detector=useDetector?new BarcodeDetector({formats:["ean_13","ean_8","code_128","code_39","qr_code","upc_a","upc_e"]}):null;const zxr=useDetector?null:new MultiFormatReader();let scanning=true;window._sgStopRecCamera=()=>{scanning=false;stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);};const scan=async()=>{if(!scanning)return;if(video.readyState>=2&&video.videoWidth>0){canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.drawImage(video,0,0);try{if(useDetector){const codes=await detector.detect(canvas);if(codes.length>0&&scanning){window._sgStopRecCamera();handleRecScan(codes[0].rawValue);return;}}else{const bmp=new BinaryBitmap(new HybridBinarizer(new HTMLCanvasElementLuminanceSource(canvas)));const result=zxr.decode(bmp);if(result&&scanning){window._sgStopRecCamera();handleRecScan(result.getText());return;}}}catch(e){}}if(scanning)setTimeout(scan,100);};scan();}catch(e){setRecCameraError(e.name==="NotAllowedError"?"Camera access denied.":"Camera not available. Type SKU below.");setRecCameraActive(false);}}
async function startCameraScan(){setCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:"environment"}});setCameraActive(true);await new Promise(res=>setTimeout(res,200));const video=document.getElementById("sg-camera-feed");if(!video){stream.getTracks().forEach(t=>t.stop());setCameraActive(false);return;}video.srcObject=stream;await video.play();await new Promise(res=>setTimeout(res,1500));const canvas=document.createElement("canvas");const ctx=canvas.getContext("2d");const useDetector="BarcodeDetector" in window;const detector=useDetector?new BarcodeDetector({formats:["ean_13","ean_8","code_128","code_39","qr_code","upc_a","upc_e"]}):null;const zxr=useDetector?null:new MultiFormatReader();let scanning=true;window._sgStopCamera=()=>{scanning=false;stream.getTracks().forEach(t=>t.stop());setCameraActive(false);};const scan=async()=>{if(!scanning)return;if(video.readyState>=2&&video.videoWidth>0){canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.drawImage(video,0,0);try{if(useDetector){const codes=await detector.detect(canvas);if(codes.length>0&&scanning){window._sgStopCamera();handleScan(codes[0].rawValue);return;}}else{const bmp=new BinaryBitmap(new HybridBinarizer(new HTMLCanvasElementLuminanceSource(canvas)));const result=zxr.decode(bmp);if(result&&scanning){window._sgStopCamera();handleScan(result.getText());return;}}}catch(e){}}if(scanning)setTimeout(scan,100);};scan();}catch(e){setCameraError(e.name==="NotAllowedError"?"Camera access denied. Please allow camera access.":"Camera not available. Use the text input below.");setCameraActive(false);}}
  function stopCamera(){
