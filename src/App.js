...
function AppInner({role,onLogout,TABS,userId}){
  ...
  async function startRecCameraScan(){setRecCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:true});setRecCameraActive(true);let video=null;for(let i=0;i<30;i++){await new Promise(res=>setTimeout(res,100));video=document.getElementById("sg-rec-camera-feed");if(video)break;}if(!video){stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);return;}video.srcObject=stream;await video.play();await new Promise(res=>{if(video.videoWidth>0)res();else{video.addEventListener("loadedmetadata",res,{once:true});setTimeout(res,2000);}});const canvas=document.createElement("canvas");const ctx=canvas.getContext("2d");const zxr=new MultiFormatReader();let scanning=true;window._sgStopRecCamera=()=>{scanning=false;stream.getTracks().forEach(t=>t.stop());setRecCameraActive(false);};const scan=()=>{if(!scanning)return;if(video.readyState>=2&&video.videoWidth>0){canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.drawImage(video,0,0);try{const id=ctx.getImageData(0,0,canvas.width,canvas.height);const src=new RGBLuminanceSource(id.data,canvas.width,canvas.height);const bmp=new BinaryBitmap(new HybridBinarizer(src));const result=zxr.decode(bmp);if(result){window._sgStopRecCamera();handleRecScan(result.getText());return;}}catch(e){}}if(scanning)requestAnimationFrame(scan);};requestAnimationFrame(scan);}catch(e){setRecCameraError(e.name==="NotAllowedError"?"Camera access denied.":"Camera not available. Type SKU below.");setRecCameraActive(false);}}
    try{
      ...
      await video.play();
      async async function startCameraScan(){setCameraError("");try{const stream=await navigator.mediaDevices.getUserMedia({video:true});setCameraActive(true);await new Promise(res=>setTimeout(res,200));const video=document.getElementById("sg-camera-feed");if(!video){stream.getTracks().forEach(t=>t.stop());setCameraActive(false);return;}video.srcObject=stream;await video.play();await new Promise(res=>{if(video.videoWidth>0)res();else{video.addEventListener("loadedmetadata",res,{once:true});setTimeout(res,2000);}});const canvas=document.createElement("canvas");const ctx=canvas.getContext("2d");const zxr=new MultiFormatReader();let scanning=true;window._sgStopCamera=()=>{scanning=false;stream.getTracks().forEach(t=>t.stop());setCameraActive(false);};const scan=()=>{if(!scanning)return;if(video.readyState>=2&&video.videoWidth>0){canvas.width=video.videoWidth;canvas.height=video.videoHeight;ctx.drawImage(video,0,0);try{const id=ctx.getImageData(0,0,canvas.width,canvas.height);const src=new RGBLuminanceSource(id.data,canvas.width,canvas.height);const bmp=new BinaryBitmap(new HybridBinarizer(src));const result=zxr.decode(bmp);if(result){window._sgStopCamera();handleScan(result.getText());return;}}catch(e){}}if(scanning)requestAnimationFrame(scan);};requestAnimationFrame(scan);}catch(e){setCameraError(e.name==="NotAllowedError"?"Camera access denied. Please allow camera access.":"Camera not available. Use the text input below.");setCameraActive(false);}}
  function stopCamera(){
