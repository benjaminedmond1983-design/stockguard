...
const TAB_GUIDE = [
  ...
  { id:'quickbooks', name:'QuickBooks', role:'Owner', description:'Sync your StockGuard sales and expenses to QuickBooks Online so your books stay current without double entry.', actions:['Connect or disconnect your QuickBooks company','Push sales to QuickBooks with one click','Confirm your connected Company ID'], tip:'After connecting, sync once and check that a recent sale shows up in QuickBooks before you rely on it.' },
  { id:'square', name:'Square', role:'Owner', description:'Connect your Square POS so in-store sales and products flow into StockGuard automatically.', actions:['Connect your Square account and location','Import Square products into StockGuard','Import Square sales history'], tip:'If you sell on both Square and the floor, import Square first so your product list starts complete.' },
  { id:'clover', name:'Clover', role:'Owner', description:'Connect your Clover POS to sync items and push stock updates back to your register.', actions:['Connect your Clover account','Import Clover items into StockGuard','Push stock quantities back to Clover'], tip:'Decide which system owns your counts - let StockGuard push to Clover so your register never shows stale stock.' },
  ...
  { id:'tax-center', name:'Tax Center', role:'Owner', description:'Year-end financial reports and accountant-ready exports, generated from your real sales and inventory.', actions:['See sales, COGS, inventory valuation, and vendor spend for the year','Download every report as a CSV for your accountant','Ask the AI Financial Assistant plain-English questions about your numbers'], tip:'At year-end, hit Download All Reports and hand the CSVs to your accountant, then use the AI Assistant to sanity-check your position before you file.' },
  ...
  { id:'billing', name:'Billing', role:'Owner', description:'Manage your subscription, payment method, and invoices.', actions:['Update your card or payment method','View and download past invoices','Change or cancel your plan'], tip:'Keep your payment method current - a failed charge can pause the AI features you rely on.' },
];
...
export default function HelpPanel() {
  ...
  return (
    <>
      <button onClick={() => setOpen(true)} aria-label="Open StockGuard help guide" style={{ position:'fixed', bottom:24, right:24, width:48, height:48, borderRadius:'50%', background:NAVY, color:'#fff', border:'none', fontSize:20, fontWeight:600, cursor:'pointer', boxShadow:'0 4px 14px rgba(27,43,75,0.35)', zIndex:1000 }}>?</button>
      ...
      {open && (
        <div onClick={() => setOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.45)', zIndex:1001, display:'flex', justifyContent:'flex-end' }}>
          <div onClick={e => e.stopPropagation()} style={{ width:'min(720px,100%)', height:'100%', background:BG, boxShadow:'-8px 0 30px rgba(0,0,0,0.2)', display:'flex', flexDirection:'column', animation:'sg-help-slide-in 0.22s ease-out' }}>
            <style>{`@keyframes sg-help-slide-in{from{transform:translateX(40px);opacity:0}to{transform:translateX(0);opacity:1}}`}</style>
            ...
            <div style={{ flex:1, display:'flex', minHeight:0 }}>
              <nav style={{ width:200, flexShrink:0, background:NAVY, overflowY:'auto', padding:'8px 8px 16px' }}>
                {GROUPS.map((group, gi) => (
                  <div key={group.label}>
                    <div style={{ fontSize:9, fontWeight:700, color:'rgba(255,255,255,0.3)', letterSpacing:'0.08em', textTransform:'uppercase', padding: gi===0 ? '12px 12px 5px' : '16px 12px 5px' }}>{group.label}</div>
                    {group.ids.map(id => {
                      const tab = TAB_GUIDE.find(t => t.id === id);
                      if (!tab) return null;
                      const isActive = activeId === tab.id;
                      return (
                        <button key={tab.id} onClick={() => setActiveId(tab.id)} style={{ display:'block', width:'100%', textAlign:'left', padding:'8px 12px', marginBottom:1, borderRadius:7, border:'none', fontSize:12.5, cursor:'pointer', background: isActive ? 'rgba(255,255,255,0.16)' : 'transparent', color: isActive ? '#fff' : 'rgba(255,255,255,0.65)', fontWeight: isActive ? 600 : 400 }}>
                          {tab.name}
                        </button>
                      );
                    })}
              ...
              <div style={{ flex:1, overflowY:'auto', padding:'28px 32px' }}>
                ...
                <ul style={{ paddingLeft:18, marginBottom:22 }}>
                  {active.actions.map(a => (<li key={a} style={{ fontSize:13.5, color:'#3A4357', marginBottom:6, lineHeight:1.5 }}>{a}</li>))}
                </ul>
            ...
            <div style={{ textAlign:'center', fontSize:11, color:'#8A93A6', padding:'10px 0', borderTop:'1px solid rgba(27,43,75,0.08)' }}>Proverbs 16:3</div>
          </div>
        </div>
      )}
    </>
