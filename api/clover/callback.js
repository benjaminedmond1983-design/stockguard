export default async function handler(req, res) {
  ...
  try {
    ...
    if (!code) {
      ...
      const authUrl = `${CLOVER_BASE}/oauth/v2/authorize?client_id=${CLOVER_APP_ID}&redirect_uri=${redirectUri}`;
    ...
    const tokenRes = await fetch(`${CLOVER_BASE}/oauth/v2/token`, {
      ...
      body: JSON.stringify({ client_id: CLOVER_APP_ID, client_secret: CLOVER_APP_SECRET, code: code }),
    });
